...
class FetchImages extends Component {
  ...
  async showImages(breed) {
    ...
    this.setState({ ...this.state, newData: items })
  }
    ...
    return (
      <div>
        <input className="txt"
          ...
          onChange={e => this.setState({ ...this.state, breed: e.target.value })}
        />
        ...
        <div>
          <Fvrt favrtImages={this.state.favData} handleDelete={handleDelete} />
        </div>
      </div>
    );
  }
